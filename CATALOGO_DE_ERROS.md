...
**Gate V100 local aprovado:** domínio 58/58, UI 102/102 em desktop/mobile, compatibilidade V99 80/80 lógica e 57/57 UI, V89/V96/V97/V98 UI 41/41 e regressão crítica 610/610. `permission-denied`, timeout e offline retornam erro com código, zero transação/escrita e restauração do botão. `firestore.rules` permaneceu no SHA-256 `5bd436...`; `calendario.html` e `calendarios.html` permaneceram idênticos no SHA-256 `9fc8a2...`. Estado máximo antes do upload: corrigido e validado localmente/pronto para publicação; produção continua V99.
