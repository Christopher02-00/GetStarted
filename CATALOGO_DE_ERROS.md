...
### 70.37 A Central aceitava mensalista ativo sem contrato e bloqueava a correção depois da entrada

**Defeito reproduzido na produção em 19/08/2026:** a Central de Clientes mostrava iPhone Campo Largo como mensalista ativo, porém com plano legado, mensalidade R$ 0,00 e sem vencimento. Ao clicar em **Editar dados → Abrir valor deste contrato**, a tela de Contratos abria sem esse cliente e informava que o contrato não apareceu entre os ativos. A Amanda e o Chris não tinham formulário para concluir o valor. A ficha operacional também exigia valor/vencimento positivos para salvar qualquer outro campo, apesar de esses dados estarem somente leitura.

**Causa comprovada:** a projeção da Central une cadastro, configuração, carteira, acesso e base legada, podendo reconhecer uma identidade operacional ativa mesmo sem `contratos_cliente/{slug}`. Contratos e Mensalidades, porém, nasciam somente da coleção financeira. A entrada unificada já cria toda a estrutura, mas registros legados ou reativações incompletas não possuíam reconciliação: o mensalista existia para operação/calendário e não existia para o Financeiro.

**Correção local V92:** Valores dos contratos cruza a própria projeção confirmada da Central com `contratos_cliente`. Mensalista ativo sem documento recebe um cartão explícito de **estrutura financeira incompleta**. Amanda ou Chris informam plano, valor, vencimento, primeira competência e cortesia; uma única transação relê as fontes, recusa avulso/arquivado/inativo, impede contrato concorrente, cria o contrato canônico e a mensalidade determinística e atualiza apenas campos financeiros de fichas já existentes. Portal, calendário, Stories, tokens e contato não são criados nem alterados. Um contrato pausado/encerrado divergente não é reativado automaticamente. A ficha operacional deixou de exigir valor inexistente para salvar campos não financeiros e não grava zero sobre o legado.
...
**Prova real complementar em 21/08/2026:** na V98 pública, que preserva a correção V95, Chris operou Gabrielle após autorização específica para um conteúdo interno. `postagens/video_0OlGgdUbNSNRT0GkIhg1` avançou para `aguardando_agendamento`, a fila de legendas caiu de 21 para 20 após recarga e o mesmo item apareceu com a legenda persistida em `Agendar postagens (2)` da Cecília. O recibo append-only `CFmHEYC5H2GhiElhHYMp` registrou Chris, Gabrielle, `runTransaction` e apenas o caminho da postagem. A regra V96 publicada foi lida: Gabrielle pode atualizar `postagens`, não integra a permissão geral de atualização de `videos_producao`, e o log delegado não admite update/delete. O vídeo observado não tinha campo `legenda`; como não houve snapshot anterior, não se afirma igualdade byte a byte. **Estado:** 70.42 encerrado para essa jornada autorizada; nenhum agendamento ou tentativa proibida de escrita no vídeo foi executado.

### 70.43 O login Google não concluía nos navegadores controlados pelo Codex

**Defeito reproduzido na V96 publicada em 21/08/2026:** no navegador interno, o gate conclui a preparação, o botão passa para **Continuar com Google** e, após o clique, mostra **Abrindo Google…**. Nenhuma janela de seleção controlável permanece e o Firebase retorna `auth/popup-closed-by-user`. A falha acontece dentro de `signInWithPopup()` antes de conta, usuário, consulta ao Firestore ou regra. Em paralelo, o Chrome 151 e a extensão existem localmente, mas o transporte do Codex informa que o Chrome não está disponível.

**Causas separadas:** o site V96 possui somente o caminho popup e não oferece retorno por redirect para a superfície interna atual. O mecanismo externo que encerra o popup continua desconhecido; não é correto atribuí-lo ao build ou ao Firestore. A indisponibilidade do Chrome é outra causa, na conexão Codex ↔ extensão, e não deve produzir alteração no site nem reparo manual do native host.

**Correção local V97:** `signInWithPopup` permanece a primeira opção e a primeira operação assíncrona do clique. Depois de popup bloqueado/encerrado, a interface revela a segunda ação explícita **Entrar nesta aba**; nunca redireciona automaticamente. Esse gesto chama `signInWithRedirect`, e a inicialização consome `getRedirectResult` uma vez, com sentinela de dez minutos, timeout, limpeza e mensagens com código `auth/*`, sem loop. Conta não autorizada, sessão existente e logout preservam as barreiras. `firestore.rules` e dados operacionais não mudaram.

> **LEI:** falha pré-credencial do provedor não é falha de Firestore. Compatibilidade alternativa de login deve preservar o caminho normal, exigir gesto explícito para redirect, processar o retorno uma vez e nunca criar loop, ampliar conta autorizada ou esconder o código real do erro.

**Gate de regressão:** `scripts/regression-v97-login-google.mjs` aprova 44 verificações e `scripts/regression-v97-ui-login-google.mjs` aprova 10 interações desktop/mobile: popup normal, fechamento/bloqueio sem redirect automático, segundo gesto explícito, retorno/cancelamento/erro/timeout/expiração, conta não autorizada, sessão existente, logout, clique duplo e ausência de loop. O frontend V97 foi comprovado publicado e preservado na V98. Depois da reconexão do Chrome, o popup normal concluiu como Chris e permaneceu autenticado após recarga; a jornada real do fallback por redirect no navegador interno ainda não foi exercitada.

### 70.44 O perfil real do Chris abriu praticamente em branco após a V96

**Incidente observado visualmente em 21/08/2026:** no Safari, a autenticação concluiu e o seletor confirmou `Chris`, mas a sidebar exibiu somente a marca, sem navegação, e a área principal permaneceu vazia. Um toast informou que a entrada ocorreu, porém parte dos dados não carregou. O relato do usuário foi de que a mudança “apagou tudo”, mas a captura prova apenas ausência visual e falha pós-login; não prova exclusão de qualquer documento.

**Causa comprovada na V97 publicada:** a V96 substituiu o modo de auditoria V95 pelo modo de operação real e definiu `atualizarBannerOperacaoPerfilChris()`, mas deixou em `mudarUsuarioGlobal()` uma chamada residual para `atualizarBannerAuditoriaChris()`. Essa função já não existia. O `ReferenceError` acontece depois de definir `usuarioAtual` e antes de remover `body.semUsuario`, reconstruir DOM, iniciar listeners ou alcançar a primeira leitura. A V97 herdou o mesmo corpo; por isso o seletor confirma Chris enquanto menus e conteúdo permanecem ocultos. Não existe caminho de exclusão nessa fase.

> **LEI:** perfil visualmente vazio nunca autoriza restauração, recriação, migração ou ampliação de regra. Primeiro provar se os documentos existem e qual etapa falhou. Sem delegação, Chris recebe sua visão normal completa; retorno/reload/logout não persistem papel operado; erro deve ser indisponibilidade explícita, não ausência de dados.

**Correção e prova V98:** a chamada residual foi substituída pela função operacional vigente, sem optional chaining ou silêncio de erro. `regression-v98-perfil-chris.mjs` executa a fase real que quebrava e aprova 20/20; a UI sintética aprova 12/12 em desktop/mobile, Chris normal, delegação e retorno idempotente; a crítica aprova 610/610. `firestore.rules`, calendários e dados não mudaram na correção. Depois do upload, o HTML público coincidiu byte a byte com o pacote (`af7fb052...`), o popup normal autenticou Chris no Chrome, 35 itens de navegação e o conteúdo normal apareceram sem erro/warning capturado nessa jornada, e uma nova recarga preservou a visão. Nenhum perfil delegado ou salvamento operacional foi executado; esses pontos permanecem em O1/V96.

> **LEI COMPLEMENTAR:** renomear um modo operacional exige atualizar e executar o inicializador canônico; sintaxe válida e testes que substituem o inicializador por stub não detectam identificador órfão. A regressão deve atravessar identidade → banner → remoção de `semUsuario` → isolamento do DOM antes de qualquer leitura.

### 70.45 Quantidade positiva podia ser salva com uma ordem de captação vazia

**Defeito observado e reproduzido em 21/08/2026:** a tela móvel do Luís mostrou Place, sessão `place|2026-08|S02`, cinco vídeos esperados e zero títulos liberados. O escritor de agendamento e o replanejamento aceitavam `sessaoPlanejamentoVersao:1` junto de `sessaoItensPlanejados:[]`, sem extras autorizados. O sugeridor ainda convertia zero itens em quantidade `1`. Depois, o leitor respeitava corretamente o snapshot moderno vazio e bloqueava o envio para não capturar conteúdo de outra sessão.

**Falha de compatibilidade associada:** a lei Vital Seg/Helo exige versão 1 **e** array para uma sessão moderna, mas o predicado usava `&&` entre as ausências. Um documento com somente um dos marcadores perdia a porta legada, embora a migração não estivesse completa. O print não autoriza afirmar qual formato exato existe no documento Place; nenhum dado real foi lido ou alterado nesta correção.

**Correção local V99:** a classificação moderna foi centralizada na coexistência dos dois marcadores. Criação e replanejamento usam o mesmo gate e não persistem snapshot vazio sem autorização explícita de extras. A quantidade de uma sessão moderna normal deriva dos títulos exatos. Replanejamento relê agendamento e calendário em transação, recusa assinatura antiga/duas abas e só grava o conjunto que a coordenação acabou de confirmar. A interface distingue calendário indisponível, legado isolado e plano moderno vazio; Amanda, Cecília ou Chris recebe o caminho de recuperação, enquanto Luís/Nathan continuam sem texto livre moderno.

> **LEI:** quantidade planejada não é autorização de captação. Sessão moderna exige versão e array juntos; sem ao menos um item exato, o escritor falha fechado, salvo extras explicitamente autorizados. Snapshot moderno vazio já existente nunca vira legado automaticamente e só é reparado por replanejamento humano transacional.

**Gate de regressão local aprovado:** `scripts/regression-v99-captacoes-videomaker.mjs` aprovou 75/75 verificações de contrato moderno/legado, snapshots malformados e duplicados, escritores, aliases, contagem, concorrência, extras estritamente booleanos e invariantes de regras/calendários. `scripts/regression-v99-ui-captacoes-videomaker.mjs` aprovou 55/55 interações em Chrome headless, desktop e mobile, com papéis, vazio moderno, erro distinto, legado isolado e plano com cinco títulos. A crítica aprovou 610/610 e as regressões selecionadas de segurança, calendários, papéis, login, Chris e legendas permaneceram aprovadas. O único `preflight` vermelho continua sendo a cópia histórica `regression-critical.mjs` na raiz, incidente já conhecido e preservado por decisão expressa.

**Prova publicada V99 e limite histórico:** o runtime público V99 coincidiu por build/patch `2026-08-21-planejamento-sessoes-v99` e SHA-256 `73b23eda...`. No Chrome autenticado como Chris, a prévia do agendamento Place `5KiwYcVXns1Sh1WFG3Mp`, S02, bloco 2, encontrou zero títulos exatos e recusou salvar com “Nenhum dado foi alterado”; nenhuma confirmação final, transação ou escrita ocorreu. Os 404 de `AGENTS.md` e scripts eram saída intencional do Pages por `_config.yml`, não prova de ausência na árvore GitHub. A decisão V100 posteriormente removeu a pauta como pré-condição da baixa factual.

**Decisão operacional V100 — regra V99 parcialmente superada:** o usuário esclareceu que a pauta da Place atrasou e que a organização prévia por Amanda/Cecília transformou um controle editorial em bloqueio do fato já ocorrido. A V99 impediu snapshots falsos e captação de outra sessão, mas manteve o filmmaker dependente de um replanejamento humano. Essa dependência foi removida sem desfazer o isolamento conquistado na V99.

**Correção local V100:** somente Chris, Amanda ou Cecília cria e atribui uma nova sessão. Luís/Nathan continuam limitados à própria equipe e, numa sessão já atribuída, podem registrar títulos factuais mesmo se o calendário estiver ausente, vazio, inconsistente ou indisponível. Checklist exato continua opcional e transacional; somente seus itens revalidados recebem `gravado`. Títulos declarados usam `vinculoSessao: declarado_em_campo`, IDs de calendário nulos e nunca alteram outra pauta. Os vídeos seguem uma única vez para `aguardando_edicao`, fila posterior da Amanda; diferença entre esperado e realizado fica no controle e não cria aprovação nem tarefa automática para Cecília.

> **LEI V100:** calendário editorial orienta e concilia, mas não autoriza o registro factual de uma sessão válida. Quantidade é estimativa, não permissão. Filmmaker não cria sessão nem escolhe cliente transversalmente; ele baixa apenas sessão atribuída. Erro de leitura nunca vira vazio, e declaração sem calendário nunca marca pauta por nome ou suposição.

**Gate V100 aprovado e publicado:** domínio 58/58, UI 102/102 em desktop/mobile, compatibilidade V99 80/80 lógica e 57/57 UI, V89/V96/V97/V98 UI 41/41 e regressão crítica 610/610. `permission-denied`, timeout e offline retornam erro com código, zero transação/escrita e restauração do botão. `firestore.rules` permaneceu no SHA-256 `5bd436...`; `calendario.html` e `calendarios.html` permaneceram idênticos no SHA-256 `9fc8a2...`. O runtime V100, `main@ad1a3df` com 19/19 artefatos e Pages `32517040855` foram comprovados; a baixa real Luís/Nathan → Amanda/Controle continua pendente.

### 70.46 Cecília não possuía uma visão mensal confiável de conteúdos concluídos e pendentes

**Lacuna operacional confirmada na arquitetura V100:** Calendários possuía uma visão resumida de flags da pauta e controles separados de montagem, revisão e postagem, mas não cruzava de forma canônica cada `itemId` com o vídeo e a postagem correspondentes. Cecília não tinha um painel único por cliente e competência, nem uma conferência administrativa auditável. A virada do mês podia esconder a urgência de itens anteriores, e usar apenas `gravado`, `agendado`, `posted` ou um checkbox como conclusão confundiria andamento, término operacional e conferência humana.

**Correção V101:** a mesma aba Calendários ganhou o submodo **Controle de conclusão**, presente no DOM somente para Cecília e Chris. O progresso automático deriva, por identidade estável `calendarId + competência + itemId`, das fontes existentes de calendário, `videos_producao` e `postagens`. Publicação ou uma finalização direta explicitamente reconhecida concluem o fluxo; captação, edição, aprovação, legenda e agendamento permanecem etapas intermediárias. A conferência da Cecília é uma projeção separada e não autoritativa, com assinatura SHA-256 da fonte, revisão, operação idempotente, autoria real, papel operado, horário, observação e evento append-only. Somente Chris, no próprio perfil, pode encerrar excepcionalmente um item ainda pendente, sempre com motivo e histórico reversível.

**Fronteiras preservadas:** renderizar ou recarregar o painel nunca cria documento. Ações escrevem apenas em `calendarios_conferencias` ou `calendarios_encerramentos`, sob transação que cria retrato e evento no mesmo commit; calendário, sessão, vídeo, postagem, Portal e autorização de captação não são modificados. Item legado sem `itemId` é somente leitura e nunca produz 100% verificado. Erro, timeout, cota, permissão, alias ambíguo, vínculo duplicado ou fonte parcial aparecem como indisponibilidade/inconsistência e bloqueiam ações; não viram 0%. Pendências de competências anteriores permanecem acima até conclusão operacional e conferência.

**Proteção da V100/Place:** produção declarada em campo com `calendarItemId` nulo continua sem vínculo e não é conciliada por título. Os escritores posteriores de postagem copiam o vínculo canônico quando ele existe e falham fechados quando a origem é ausente, manual ou pertence a outro cliente. O painel apenas consome o estado e jamais decide quais captações são permitidas ou altera `sessaoPlanejamentoVersao`, `sessaoItensPlanejados`, `calendarItemId` ou `calendarItemIdx`.

> **LEI V101:** progresso operacional e conferência administrativa são estados independentes. Renderizar o controle nunca cria documento; conferir nunca altera o workflow. Identidade estável vence título, índice e posição. Produção V100 sem vínculo nunca é ligada à pauta por título. Erro não é 0%, legado sem ID não é 100% e pendência não expira na virada do mês.

**Gate local V101 aprovado:** `scripts/regression-v101-controle-conclusao.mjs` aprovou 128/128 verificando identidades, estados, fontes, SHA-256, transação, recibo, retry, clique duplo, duas abas, papéis, histórico, legado, erros e preservação V100. `scripts/regression-v101-ui-controle-conclusao.mjs` aprovou 127/127 em desktop e mobile, sem overflow ou erro de página. As regras passaram 46/46 no Firestore Emulator isolado `demo-get-conclusao-v101`, com zero ocorrência do limite de 1.000 expressões. Regressão crítica 610/610, V100 58/58, V99 80/80, login V97 44/44, perfil V98 23/23, segurança 20/20 e Calendários V87 50/50 permaneceram aprovadas. O único vermelho do `preflight` é a cópia histórica `regression-critical.mjs` na raiz, preservada por decisão expressa e excluída do Pages. Neste gate, V101 está validada localmente; publicação das regras, deploy do frontend e jornada autenticada ainda não são fatos.

### 70.47 Calendários legados sem `itemId` ficavam permanentemente fora da conferência

**Causa confirmada na V101 publicada:** o Controle de conclusão encontrou itens antigos sem `itemId` e corretamente os manteve como `legado — somente leitura`. A barreira impediu vínculos perigosos por título ou posição, porém grande parte do histórico não podia receber conferência administrativa nem participar de 100% verificado.

**Correção local V102:** Calendários → Controle de conclusão ganhou uma ferramenta exclusiva do Chris real. A prévia conta calendários e itens sem ID sem escrever nem copiar títulos para recibos. A aplicação processa um calendário por transação: relê o documento, preserva IDs existentes, gera IDs determinísticos e distintos somente para ausências, grava backup integral em `calendarios_versoes`, atualiza o calendário por merge e cria recibo append-only em `calendarios_itemid_migracoes/{calendarId}/operacoes/{operationId}`. Item excluído também recebe identidade para não voltar como legado ao ser restaurado. ID inválido, duplicado, colisão, falha ou concorrência bloqueiam fechado; retry é idempotente e resultado parcial permanece explícito e retomável.

**Barreiras permanentes:** abrir/renderizar nunca migra; título, índice e nome não associam produção. O editor preserva IDs já presentes no servidor e recusa uma aba antiga que tentaria removê-los. Resgate e varredura normalizam itens restaurados sem criar conferência. As regras exigem Chris, campos restritos, `operationId`, contagens, SHA-256, horário do servidor, backup e recibo no mesmo commit; recibos são imutáveis e delete físico permanece proibido. Rollback de frontend/regra não remove IDs já usados por conferências.

> **LEI V102:** compatibilidade de leitura não substitui migração de identidade. Normalização de `itemId` é uma ação explícita, auditável, idempotente e reversível por backup; nunca acontece durante renderização e nunca usa título como vínculo.

**Gate local V102 aprovado:** `scripts/regression-v102-itemids-legados.mjs` aprovou 39/39; `scripts/regression-v102-ui-itemids-legados.mjs` aprovou 22/22 em desktop/mobile; regras V101+V102 aprovaram 55/55 no Firestore Emulator com `expression_limit_hits=0`. V101 128/128, V100 58/58, V99 80/80 e crítica 610/610 permaneceram aprovadas. Produção continua V101 até o usuário publicar a V102 e, depois, executar conscientemente a prévia e a migração no perfil real do Chris.

### 70.48 O Financeiro misturava competência, caixa e ciclo atual e criava cobranças ao abrir a tela

**Defeitos confirmados na V102 e na auditoria financeira somente leitura:** Mensalidades materializava documentos de `pagamentos_mensais` durante a renderização; Financeiro, Mensalidades, Régua e Contratos calculavam totais por bases temporais diferentes; a Régua misturava passivo anterior, mês selecionado e previsão; pagamento atrasado podia ser confundido com receita da competência em que entrou no caixa; e o ciclo do cliente representava prioritariamente o estado atual, insuficiente para preservar hiatos de saída e reativação. Alias, duplicata ou estado terminal divergente ainda podiam ser escolhidos silenciosamente por um consumidor. A abertura de uma tela, portanto, podia mudar o banco e duas superfícies legítimas podiam mostrar números incompatíveis.

**Correção local V103:** `financeiro-core.mjs` tornou-se a projeção pura e compartilhada de vigência, valor, obrigação, competência, caixa, Régua e carteira mensal. `financeiro-ui-v103.mjs` integra as quatro telas sem escrita na renderização: obrigação ausente aparece como derivada e só é materializada por ação explícita. A Régua separa atrasos reais anteriores, competência selecionada e próxima previsão. Caixa usa a data efetiva; obrigação e quitação permanecem na competência contratual. `vigencias[]` aceita múltiplos intervalos e preserva o hiato de reativação; legado continua leitura direcional sem migração silenciosa. Conflito de identidade, vigência, status ou pagamento falha fechado e nunca vira zero.

**Escrita e segurança:** alterações de contrato usam revisão monotônica, operação determinística, motivo, competência inicial e evento em `clientes_ciclo_financeiro` no mesmo commit. Mensalidades pagas/canceladas não são reescritas; futuras abertas ou isentas podem acompanhar valor programado sob regra restrita. Salários, custos e ajustes usam `financeiro_lancamentos`, separados de mensalidades e extras, e a baixa exige data real. Ledger é exclusivo do Chris e append-only; delete físico permanece proibido; contato financeiro continua em coleção privada e nunca é copiado para código, fixture, relatório ou Portal.

**Correção dirigida preservada atrás de prévia:** o Financeiro do Chris possui uma prévia zero-write que só libera a ação se confirmar os contratos físicos canônicos, ausência de pagamento incompatível, saídas autoritativas, empresas Joaquim separadas e projeção de setembro com 19 ativos. A transação programa Vitalle em R$ 1.000 desde setembro, fecha Monique, Joaquim Assados e Açougue São Joaquim em agosto, arquiva por soft-delete a duplicata de saída e guarda o contato da Zeiss somente na agenda privada. Pagos, cancelados e históricos são preservados. Mudança entre prévia e commit, recibo parcial ou releitura divergente bloqueiam retry cego.

> **LEI V103:** renderizar não grava. Competência, quitação e caixa são dimensões diferentes, mas todas as telas usam a mesma projeção. Obrigação virtual não é cobrança criada; somente documento real vencido entra no passivo. Ciclo mensal é linha temporal, não flag atual. Conflito não é zero. Alteração financeira explícita exige identidade canônica, revisão, operação idempotente e ledger, sem ampliar papéis nem tocar calendário/captação.

**Gate local V103 aprovado:** núcleo financeiro 248/248; Portal 42/42; correção dirigida 63/63; UI 181/181 em desktop/mobile; regras 91/91 no Firestore Emulator com `expression_limit_hits=0`; regras V101/V102 55/55; V102 domínio 39/39 e UI 22/22; crítica 610/610. Os testes usam fixtures e Firestore sintético/emulado. A auditoria do ambiente real foi somente leitura. Nenhuma regra V103 foi implantada e nenhum dado real foi escrito pelo Codex; o estado máximo é **corrigido e validado localmente / pronto para publicação**, e a correção dirigida só pode ocorrer depois do upload do usuário e da ação explícita de Chris.

### 70.49 A Régua fabricava atrasos antigos, ocultava contatos e a correção de setembro tratava a Fedalto como saída

**Defeitos confirmados na V103 publicada:** a Régua projetava competências anteriores ao início real da operação e podia chamar julho/agosto de atrasados mesmo após confirmação humana de cobrança e quitação. O novo agrupamento espalhava ação, contato e previsão em blocos extensos, removendo a utilidade direta do modelo anterior. A ferramenta dirigida também interpretava a Fedalto como encerrada em setembro, embora setembro fosse apenas cortesia promocional. A saída manual da Monique falhava em contrato legado porque uma mensalidade terminal do último mês era incluída numa atualização que as regras corretamente recusavam, resultando em `Missing or insufficient permissions`. Contratos com `vigencias:[]` eram tratados como incompatíveis em vez de legado direcional.

**Correção local V104:** a Régua volta a ser uma lista compacta de ação com contato visível e editável, mês, vencimento e estado na mesma linha. O marco operacional é julho de 2026 e julho/agosto confirmados ficam fora da fila acionável; nenhuma competência anterior é fabricada como atraso. Comprovante em análise, cobrança já confirmada hoje e próxima previsão continuam distintos e recolhíveis. O telefone da Zeiss é preenchido na própria interface e persiste somente em `contatos_clientes_financeiro/zeiss`, sem hardcode público.

**Fedalto e saídas:** a Fedalto permanece ativa e recebe `isento` somente em `2026-09`, com motivo de cortesia promocional e recibo. Monique, Joaquim Assados e Açougue São Joaquim continuam como saídas separadas quando a prévia real confirmar os alvos; mensalidade terminal do último mês é preservada, e somente obrigações posteriores abertas são canceladas. `vigencias` ausente ou vazio segue compatibilidade legada segura; identidade concorrente, intervalo sobreposto ou estado divergente ainda bloqueiam.

> **LEI V104:** histórico confirmado não é atraso; cortesia não é saída; contato financeiro aparece no fluxo de cobrança sem virar dado público; mensalidade terminal não é reescrita para concluir saída. Prévia zero-write, transação, recibo e releitura são obrigatórios.

**Gate local V104 aprovado:** correção dirigida 67/67; UI 187/187 em desktop/mobile, sem `pageerror` ou overflow; núcleo financeiro 248/248; Portal 42/42; crítica 610/610; regras 94/94 no Firestore Emulator com `expression_limit_hits=0`. Os testes usam fixtures sintéticas e projeto emulado, sem Firebase real. A V104 somente altera dados após publicação das regras e ação explícita do Chris; até lá, o estado é **corrigido e validado localmente / pronto para publicação**.

**Continuação observada após a publicação V104:** o usuário declarou ter salvo manualmente a saída da Monique, o valor futuro da Vitalle e o contato financeiro da Zeiss. Mesmo assim, a prévia V104 continuou descrevendo o plano original, exibiu o campo do contato vazio e permaneceu bloqueada por ciclos legados. O print comprova o frontend V104 e esse comportamento visual; não comprova sozinho o conteúdo exato dos documentos reais. O receio de duplicação era legítimo porque a correção não reconciliava cada alvo nem lia a agenda privada pelo mesmo resolvedor usado na Régua.

**Causa e correção local V105:** `lerCorrecaoSetembro()` construía um retrato sem integrar a fonte canônica de contatos e a UI apresentava intenção, não o estado final por alvo. A V105 centraliza a resolução do contato para Régua, WhatsApp e correção dirigida; o documento canônico privado vence, legado só entra quando inequívoco e conflito bloqueia. Cada alvo passa a ser `já confirmado`, `ajuste pendente`, `bloqueado` ou `indisponível`. Estado completo equivalente gera no-op; somente pendências seguras entram na transação. Mesma competência/data sem equivalência de motivo, pendências ou `fichaSnapshot` continua bloqueando com zero escrita.

> **LEI V105:** correção dirigida reconcilia o estado canônico atual antes de escrever. Contato privado já confirmado é reconhecido, nunca solicitado ou gravado de novo; alegação manual não substitui prova completa. Tudo confirmado elimina a ação, somente pendências entram no commit e divergência histórica ou concorrente falha fechado.

**Gate V105 comprovado publicado antes da V106:** compatibilidade V104 68/68; conciliação V105 84/84; UI V105 de contatos/Régua 86/86 desktop/mobile; UI V104 187/187; núcleo 248/248; Portal 42/42; crítica 610/610; Emulator financeiro/contatos 110/110 e V101/V102 55/55, `expression_limit_hits=0`, regras inalteradas `289fdc…`; preflight funcional com somente o `FAIL` histórico conhecido da raiz. A suíte inclui corrida entre abas com alteração sintética concorrente, aborto, zero commits e preservação do valor novo. O selo V105 tem 77 arquivos, manifesto 76/76 `5b3b8376…`, ZIP `86288865…`, paridade 70/70 e rollback 7/7. Publicação não prova aplicação de dados; nenhum dado real foi escrito pela V105 por esta entrega.

### 70.50 Duas saídas divergentes do Joaquin impediam a consolidação sem permitir escolha segura

**Causa confirmada após leitura e decisão humana:** `joaquin-assados` possuía dois registros de saída ativos. Ambos apontavam para 15/09/2026 e baixo retorno financeiro, mas divergiam em campos autoritativos como aviso, motivo normalizado e competência final. A V105 recusava arquivar qualquer um por suposição, exibindo `SAIDA_DUPLICADA_DIVERGENTE` e bloqueando parte dos totais. Esse comportamento preservava o histórico, porém faltava a decisão do responsável sobre qual documento era canônico.

**Fato confirmado pelo responsável:** `clientes_encerrados/62eBY5iSyFtP21vECYMm` é o registro correto; `clientes_encerrados/0oqy4pk1tcKZccWWyZDi` é o concorrente; a saída ocorre em 15/09/2026, a última competência cobrada é 2026-08 e o motivo é baixo retorno financeiro. Açougue São Joaquim permanece uma identidade separada.

**Correção local V106:** Financeiro ganhou uma porta exclusiva do Chris e independente do lote V105. A prévia relê os dois registros exatos, ficha, contrato, mensalidades posteriores e eventos sem escrever. Se o retrato corresponder integralmente aos fatos confirmados, a transação preserva o documento canônico, aponta `clientes_config/joaquin-assados.saidaAtivaId`, arquiva somente o concorrente por soft-delete e cria o recibo determinístico `fin_v106_joaquin_saida_canonica_20260915`. Contrato, mensalidades, Açougue e demais clientes não são escritos.

**Falha fechada:** depois da confirmação humana, uma segunda leitura de pertencimento barra com zero escrita terceiro registro ou mensalidade surgidos enquanto o diálogo estava aberto; hashes transacionais barram mudança dos documentos conhecidos. Clique duplo, retry e duas abas convergem; estado final já confirmado é no-op e estado parcial exige auditoria. O SDK Web não possui query lock de coleção: criação manual/Admin SDK na janela residual entre a segunda consulta e o commit é detectada pela releitura final, sem sucesso falso ou retry, mas não recebe promessa impossível de zero escrita. Nenhuma regra foi ampliada e publicar o frontend não executa a conciliação.

> **LEI V106:** documento canônico concorrente é decisão humana rastreada, não inferência de software. Prévia zero-write, IDs exatos, hashes transacionais, soft-delete, operação determinística e releitura pós-commit são obrigatórios; documento canônico e fontes financeiras fora do orçamento permanecem imutáveis.

**Gate V106 validado e pacote local pronto para publicação:** regressão dirigida 193/193; UI 53/53 em Chrome real desktop/mobile sem erro de página ou overflow; V105 84/84; V104 68/68; regressões históricas alinhadas 483/483; crítica 610/610. O pacote tem 79 arquivos totais; manifesto com 78 entradas, sendo 71 publicáveis + 7 rollback V105; upload GitHub de 72 arquivos, sendo 71 publicáveis + manifesto. `firestore.rules` permanece inalterado no SHA-256 `289fdc40d5cc061463b898faae8e4571072897038238a7cf3c463462e170a558`; a prova Emulator exata desse arquivo permanece a da V105, sem nova execução apresentada como prova fresca V106. Produção permanece V105; V106 não foi publicada e nenhum dado real foi escrito pela V106.

### 70.51 A conciliação concluída continuava parecendo uma correção pendente

**Causa comprovada depois da aplicação V106:** o usuário confirmou a saída correta e a mensagem `SAIDA_DUPLICADA_DIVERGENTE` desapareceu. O cartão, porém, conservava título de confirmação, selo V106, borda amarela e botão de verificação. Apenas o filho interno de status mudava para verde. Ao recarregar, o shell nascia novamente como pendente até um clique. O teste anterior procurava botões somente dentro do filho de status e não media o cartão inteiro.

**Correção V107:** um único renderer controla o cartão completo nos estados `aguardando`, `verificando`, `pronta`, `aplicando`, `resolvida`, `bloqueada` e `indisponivel`. O estado resolvido fica verde, recebe o título `Saída correta do Joaquim confirmada`, selo `Concluído`, cursor normal e zero botão em toda a árvore. O primeiro snapshot financeiro já carregado classifica o estado no reload, sem nova fonte e sem escrita. Bloqueio ou falha de leitura ficam vermelhos e nunca parecem conclusão. O agregador geral agora considera também o resultado do Joaquim antes de declarar que todos os ajustes terminaram.

**Fronteira preservada:** V107 não muda a transação, contratos, saídas, pagamentos, regras ou papéis. Segundo clique e duas abas convergem para o cartão concluído sem novo commit. O alerta posterior `PAGAMENTO_SEM_CONTRATO_VIGENTE` é outra causa: a auditoria read-only identificou um conflito real da Fedalto em agosto repetido duas vezes na fachada. Ele continua visível e exige reparação V108 com a data real do pagamento, sem inventá-la e sem desfazer a cortesia de setembro.

> **LEI V107:** conclusão operacional precisa transformar o componente inteiro e sobreviver ao reload por derivação das fontes canônicas. Texto verde dentro de um shell pendente é estado contraditório. Retry resolvido não grava; erro diferente não reabre a correção anterior nem pode ser escondido junto com ela.

**Gate V107 local:** UI focal 111/111; UI V106 endurecida 56/56; domínio V106 193/193; V105 84/84 + UI 86/86; V104 68/68 + UI 187/187; V103 core 248/248 + UI 181/181 + Portal 42/42; V101 128/128; captações V99 80/80; Calendários V87 50/50; perfil V98 23/23; legendas V95 15/15; crítica 610/610. Chrome real sintético cobriu desktop/mobile, reload, segundo clique, duas abas, zero `pageerror`, zero overflow e papéis indevidos sem DOM/leitura. `financeiro-core.mjs` e `firestore.rules` permaneceram inalterados.

### 70.52 Um pagamento real fora da vigência aparecia repetido e bloqueava os totais sem caminho seguro de correção

**Causa confirmada por auditoria somente leitura e pelo responsável:** a Fedalto possuía mensalidades físicas de julho, agosto e setembro, mas o contrato declarava vigência apenas desde setembro. Julho estava pago e setembro era a cortesia promocional correta; agosto tinha sido marcado indevidamente como `isento`, embora R$ 1.700 tenham sido pagos em **15/08/2026**. O mesmo conflito `PAGAMENTO_SEM_CONTRATO_VIGENTE` atravessava projeções internas e podia aparecer mais de uma vez na fachada. Outro conflito histórico anterior ao início da Régua também contaminava a tela atual, apesar de continuar relevante ao consultar a própria competência antiga.

**Correção V108:** o núcleo deduplica somente conflitos estruturalmente idênticos e preserva documentos realmente distintos. A Régua atual omite conflitos explicitamente anteriores ao marco operacional de julho de 2026, sem apagá-los da auditoria da competência original. O Financeiro do Chris recebe uma porta exclusiva para a Fedalto: a prévia relê contrato, julho, agosto, setembro, saídas e ledger sem escrever; a confirmação única move o início da vigência para julho, registra agosto como pago em 15/08/2026 e cria o recibo determinístico `fin_v108_fedalto_agosto_20260815`. Julho e setembro são preconditions imutáveis e permanecem byte a byte.

**Segurança e terminalidade:** as regras aceitam somente essa transição exata do Chris, exigem contrato + agosto + recibo no mesmo commit e rejeitam data, valor, papel, campo oportunista ou intervalo divergente. Julho precisa ser o pagamento auditado de R$ 1.700 em 11/08/2026 e setembro precisa conservar exatamente a cortesia promocional; ambos são somente leitura na transação. O ledger é append-only. A exceção V108 vale somente na transição para o `operationId` V108: depois dela, edições gerenciais não financeiras e operações financeiras futuras com nova revisão e novo ledger voltam ao contrato geral, evitando congelamento. O estado `Concluído` deriva dos fatos históricos e continua terminal mesmo após uma saída futura legítima; recibo divergente nunca recebe verde. Clique duplo, retry, segunda aba e reload convergem sem botão ou nova escrita. Amanda, Cecília e demais papéis não recebem alvo, fatos, leitura ou ação V108 no DOM. Calendários, Place/Luís, captação, vídeos, Portal e as conciliações V106/V107 não são alterados.

> **LEI V108:** conflito duplicado na apresentação não é duplicata de documento e não autoriza apagamento. Pagamento histórico só entra no contrato por fato humano datado, transação estreita e recibo append-only. Deduplicação colapsa cópia integral, nunca registros distintos; filtro da Régua não apaga a auditoria histórica. Prévia é zero-write e estado concluído não conserva ação residual.

**Gate local V108:** domínio focal 27/27; UI focal 72/72 em Chrome real desktop/mobile; regras financeiras 123/123 no Firestore Emulator com `expression_limit_hits=0`; matriz histórica 12/12 suítes e 1.929/1.929 verificações, incluindo núcleo financeiro 248/248, Portal 42/42, correções V103 63/63, V104 68/68, V105 84/84 e V106 193/193, UI V103 181/181, V104 187/187, V105 86/86, V106 56/56, V107 111/111 e crítica 610/610; preflight do espelho aprovado. Uma reauditoria adversarial independente confirmou os quatro bloqueantes e cinco riscos médios anteriores corrigidos, sem novo bloqueante funcional. As provas usam fixtures sintéticas e emulador isolado. Nenhum dado real foi escrito e nenhuma regra foi publicada pelo Codex; a operação só poderá ocorrer após upload do frontend, implantação das regras V108 pelo usuário e confirmação explícita do Chris.

### 70.53 A V108 exigia setembro já canônico e, por isso, não conseguia concluir a própria correção

**Reprodução real somente leitura:** depois da publicação V108 e da liberação do Financeiro pelo Chris, o cartão da Fedalto permaneceu vermelho. A tela publicada confirmou o conflito de agosto e informou que setembro havia deixado de corresponder à cortesia promocional esperada. A releitura por `Verificar novamente sem salvar` manteve o bloqueio e executou zero escrita. O documento de setembro estava no estado legado auditado `isento + cortesia manual`, enquanto a V108 o tratava como precondition já resolvida.

**Causa:** a V108 tinha orçamento de escrita para contrato + agosto + recibo e preservava setembro byte a byte. Isso era seguro para o retrato sintético anterior, mas inviável para o retrato real: a mesma operação precisava preservar o significado de cortesia de setembro e, ao mesmo tempo, canonicalizar seus metadados. Repetir V108, editar por fora ou relaxar regras criaria estado parcial ou recibo enganoso.

**Correção V109:** o escritor V108 foi aposentado e seu ID permanece reservado. A nova operação determinística `fin_v109_fedalto_agosto_setembro_20260815` relê contrato, julho, agosto, setembro, saídas e os dois possíveis recibos. Depois da confirmação humana, escreve numa única transação somente o início da vigência, agosto pago em 15/08, setembro ainda isento com cortesia promocional explícita e o recibo V109. Julho permanece pagamento de R$ 1.700 em 11/08 e é protegido por leitura/hash, sem write. Outubro continua normal e a Fedalto não é encerrada.

**Regras e concorrência:** os três principais aceitam apenas seus próprios diffs exatos e exigem o recibo criado no mesmo `request.time`; o recibo confere os três `getAfter`, julho e ausência de recibo V108. Recibo V109 antigo sem os principais no mesmo request é recusado. Campo oportunista, data/valor/papel divergente, ausência de qualquer principal, terceiro estado, retry e sobrescrita são negados. Em duas gravações simultâneas, exatamente uma vence e a outra falha sem duplicação. Edição gerencial não financeira posterior continua permitida.

> **LEI V109:** quando uma correção histórica depende de canonicalizar uma precondition, ela passa a principal explícito do mesmo commit; não pode ser presumida resolvida nem alterada por operação paralela. Operação substituída fica reservada e sem writer. Estado concluído exige documentos e recibo equivalentes, não apenas a existência do recibo.

**Gate V109:** núcleo 29/29; UI 93/93 em Chromium desktop/mobile e papéis; Firestore Emulator 144/144 com `expression_limit_hits=0`, incluindo duas abas e recibo antigo; crítica 610/610; preflight aprovado. A produção V108 foi conferida apenas por leitura e continuava bloqueada como esperado. Nenhum dado real foi gravado e nenhuma regra V109 foi publicada pelo Codex.

**Encerramento posterior da V109:** depois da publicação comprovada do frontend e das regras pelo usuário, Chris autorizou e executou uma única confirmação. Contrato, agosto, setembro e recibo V109 foram relidos no estado final; julho permaneceu preservado. O cartão ficou terminal e sem ação também após reload. Esta causa financeira está encerrada e não pertence ao diff V110.

### 70.54 Trocar o perfil no mesmo navegador preservava estado privilegiado e uma porta da Amanda dependia de dados exclusivos do Chris

**Causa reproduzida na V109:** a limpeza de identidade era parcial entre papéis de gerência. Projeções da Central de Clientes, negócios, reuniões, fichas avulsas e painéis renderizados podiam permanecer no processo ou no DOM entre Chris e Amanda. Uma resposta assíncrona iniciada antes da troca também podia terminar depois e repovoar o estado antigo. Em paralelo, a subaba “Projetos Avulsos (produção)” aparecia para Amanda, mas seu renderer dependia de `negocios`, coleção corretamente exclusiva do Chris. O teste vermelho V110 encontrou 29 falhas em 31 verificações.

**Correção local V110:** uma fronteira síncrona única invalida a geração da identidade e limpa caches, projeções, formulários e DOM restrito antes de atribuir o novo papel ou aguardar rede. Central de Clientes, Central de Vendas e Projetos Avulsos capturam papel + geração e descartam resposta antiga. Views comerciais/financeiras exclusivas são retiradas fisicamente e restauradas por marcador. Amanda conserva a Central de Clientes operacional, mas não recebe a porta antiga de avulsos baseada em `negocios`. O retorno da Cecília restaura os links permitidos sem reload.

> **LEI V110:** trocar papel ou UID é uma fronteira de segurança, não simples filtro visual. A identidade anterior deve ser invalidada e limpa antes da rede; resposta antiga não publica cache nem DOM. Interface incompatível não se resolve ampliando permissão: recebe projeção mínima própria em causa posterior ou fica ausente.

**Gate local V110:** domínio focal 49/49; UI Chromium real 28/28 em desktop/mobile, duas abas, reload e trocas reversíveis; Financeiro core 248/248, Portal 42/42 e UIs V103–V109 preservadas; Calendários V81/V87/V101, Portal/Stories, Vendas, Place/Luís/Nathan e crítica 610/610 aprovados; preflight aprovado. `portal-cliente.html`, `avulso.html`, `firestore.rules` e módulos financeiros permanecem byte a byte V109. Nenhuma regra, dado, upload ou produção foi alterado; estado máximo é corrigido e validado localmente.

### 70.55 Um painel removido por privacidade derrubava todas as rotas permitidas da Amanda

**Relato e reprodução:** depois da V110, o perfil Chris continuava abrindo Gerência, mas Amanda não conseguia abrir Painel de controle, Gerência, a tabela Agora nem qualquer subaba permitida. A remoção física de Projetos Avulsos era correta, porém `setGerenciaSub()` continuava executando `document.getElementById('gerenciaAvulsos').style` antes de escolher a rota. Para Amanda, o nó inexistente produzia `TypeError` e interrompia a navegação antes de renderer ou leitura. O teste vermelho confirmou 7 falhas em 16 verificações estáticas e 9 em 21 no Chromium.

**Auditoria de alcance:** a mesma classe existia no roteador principal, que removia a tela atual antes de validar se a view de destino existia. Também foi encontrado `gerenciaFunilNegocios` ainda presente para Amanda, embora consuma `negocios`, e `navExtras` removido apesar de a matriz autorizar Amanda. Não há evidência de perda de dados ou negativa Firestore nesta causa: a exceção acontecia antes da rede.

**Correção local V112:** uma tabela canônica relaciona subaba e painel; o roteador gerencial percorre apenas nós existentes e valida papel/subaba antes de renderer. Funil de Negócios e Projetos Avulsos ficam fisicamente exclusivos do Chris e seus renderers negam Amanda antes da leitura. Extras volta somente para Amanda/Chris. O roteador principal valida a view antes de desmontar a tela atual, tolera callback antigo sem botão/view e explica a indisponibilidade. Chris recupera painéis exclusivos sem reload; funcionários sem Gerência continuam barrados antes de DOM, renderer e rede.

> **LEI V112:** ausência física intencional é uma fronteira válida de papel, não uma exceção de execução. Roteador autoriza e resolve o destino antes de alterar a tela atual; nó ausente falha fechado e nunca derruba outra rota permitida. Toda remoção por papel exige varrer todas as rotas permitidas, proibidas, troca de identidade, duas abas, reload e mobile.

**Gate local:** V112 estática 19/19 e UI 27/27 em Chrome real, cobrindo todas as subabas permitidas, Amanda/Chris, sete outros papéis, desktop/mobile, duas abas, reload e callback antigo sem `pageerror`. V110 49/49 + UI 28/28, V111 31/31 + UI 25/25, Portal 42/42, segurança 20/20, Financeiro 144/144 no Emulator com orçamento zero, Portal V111 56/56 no Emulator com orçamento zero, V109 UI 93/93, núcleo financeiro 248/248 e crítica 610/610 permaneceram verdes. A correção não escreve dados, não altera regras e será entregue cumulativamente com a V111; produção continua V110 até publicação e prova posterior.

### 70.56 O pedido de alteração chegava à Gabi, mas o calendário continuava bloqueado

**Relato e causa reproduzida:** Camargo's revelou o defeito, mas ele era geral. Depois que Amanda liberava um mês, o estado `liberado` bloqueava corretamente qualquer edição silenciosa. Quando o cliente usava `Enviar pedido de ajuste`, o fluxo antigo salvava a observação e criava uma demanda para Gabrielle, porém não alterava `aprovacaoMeses[competencia]`. A Gabi recebia o pedido e abria o mesmo mês ainda `liberado`; a trava correta permanecia ativa. O defeito era uma transferência incompleta de estado, não o apóstrofo do nome nem o slug `camargos`.

**Correção V113:** o pedido explícito usa operação determinística e transação única. O runtime relê o calendário, confirma sessão, slug, competência, item e estado `liberado`, muda somente aquela competência para `ajuste_interno` e cria `avisos_do_cliente/{operationId}` no mesmo commit. A interface só atualiza a memória e anuncia sucesso depois da confirmação. Retry equivalente é zero-write; clique duplo é travado antes da primeira espera. A edição seguinte da Gabi remove a aprovação antiga somente do item alterado e conserva o caminho Gabi → Amanda → cliente.

**Segurança e legado:** as regras permitem somente o contrato estreito, exigem o aviso correspondente por `getAfter`, timestamp do servidor, allowlists e a transição exata. Outro cliente, item, mês, destinatário, conteúdo simultâneo, campo extra ou estado diferente falham fechados. Comentário geral não reabre calendário. Pedido antigo, como o da Camargo's recebido antes da V113, usa um botão separado da equipe: a pessoa descreve o pedido, confirma e reabre apenas o estado mensal, sem alterar conteúdo. Legado ambíguo nunca é associado por título parecido.

> **LEI V113:** notificação não é fonte de autorização. Pedido de ajuste e transição mensal precisam ser o mesmo compromisso atômico; calendário liberado só volta a editar por pedido explícito confirmado ou recuperação humana auditável. Erro não vira sucesso, comentário não vira pedido e identidade ambígua não vira vínculo.

**Gate local V113:** domínio focal 23/23; Chrome real 26/26 em desktop/mobile, clique duplo, `permission-denied`, legado e recuperação Camargo's; regras V113 15/15, V101/V102 55/55, Financeiro 144/144 e Portal V111 56/56 no Firestore Emulator, todos com orçamento de expressões zero; Calendários V81 67/67, V83 15/15, V85 19/19, V87 50/50, carteira/papéis V89 8/8, perfis V96 28/28, migração V102 39/39, privacidade V110 49/49, Portal V111 31/31, Amanda V112 19/19, captação V99 80/80, registro V100 58/58, crítica 610/610 e preflight aprovado. Nenhum Firebase real ou documento Camargo's foi escrito; frontend e regras precisam ser publicados juntos antes da jornada real.

### 70.57 A legenda salva no calendário não transferia a postagem para a fila da Cecília

**Causa reproduzida:** existiam dois caminhos independentes. O editor do calendário gravava `items[].legenda` em `calendarios/{slug}`; a fila da Cecília lia somente `postagens` em `aguardando_agendamento`. A aprovação do vídeo sempre criava a postagem com legenda vazia em `aguardando_legenda`, mesmo quando o mesmo item já possuía legenda editorial. O manual afirmava que o texto estaria pronto, mas não existia transferência entre as fontes. Uma falha de leitura da fila também podia deixar a tela sem explicação, e o contador convertia erro em zero.

**Correção V114:** salvar a legenda como Gabrielle confirma primeiro o calendário e só então procura a postagem pelo `calendarItemId` do mesmo cliente. Legado usa apenas cliente + índice quando ambos os lados realmente não têm ID. A transação relê postagem e vídeo, confirma cliente, item, aprovação e `postagemId` canônico e move somente a postagem exata. Vínculo ausente preserva a legenda planejada; duplicidade, estado incompatível ou identidade divergente bloqueiam sem suposição. Na aprovação futura do vídeo, uma legenda editorial exata já cria a postagem em `aguardando_agendamento`. Entrega direta continua fora da fila. A tela da Cecília diferencia indisponibilidade de vazio e oferece retry.

> **LEI V114:** salvar em uma fonte não prova a transferência para outra. Legenda editorial e postagem convergem somente por identidade estável confirmada nos dois documentos e no vídeo canônico. Título, nome, dia parecido ou posição moderna nunca são identidade. Sucesso exige recibo; erro de leitura nunca vira fila vazia ou contador zero.

**Gate local V114:** reprodução vermelha na V113; domínio 28/28; Chrome real 19/19 em desktop/mobile, sucesso, planejamento, ambiguidade, `permission-denied` e fila indisponível; V95 15/15; V113 23/23; privacidade V110 49/49; Portal V111 31/31; Amanda V112 19/19; crítica 610/610; preflight com 9/9 scripts inline e resultado aprovado. `firestore.rules`, Financeiro, Portal e captação permanecem byte a byte. Os registros reais de 24/08 não foram lidos nem alterados nesta validação local.

### 70.58 Um pedido de correção ocultava o calendário e a competência errada quebrava pauta, vídeo e legenda

**Causa compartilhada comprovada:** o pedido V113 mudava corretamente a competência para `ajuste_interno`, mas os renderizadores do calendário direto e do Portal continuavam exibindo somente `liberado`. O documento permanecia no Firestore, porém o cliente via `0/0` e “calendário em produção”. Em paralelo, sessões de gravação podiam conservar a competência da data do agendamento mesmo quando a pauta pendente já estava no mês seguinte. O modo de campo então contava o mês antigo como resolvido, mostrava “tudo gravado” e deixava de carregar os `calendarItemId` necessários ao vídeo e à ponte V114 da legenda.

**Correção V115:** cada publicação da Amanda guarda uma projeção estreita da versão entregue. Durante `ajuste_interno`, cliente e Portal consultam essa versão sem aprovar de novo; calendários anteriores à V115 usam compatibilidade segura e recebem o retrato na primeira gravação da equipe. O diagnóstico de competência compara pendências reais antes de criar ou operar a sessão, sugere o mês correto e fica zero-write até confirmação. Referências aceitam HTTPS explícito ou recebem erro, nunca `href` bruto. Luís/Nathan ganham atalho direto da pauta para a sessão exata e não recebem falso estado concluído quando existe divergência.

**Simplificação de transferência:** a Gabi encerra a correção com `Enviar para Amanda revisar e publicar`. Em Calendários, Amanda recebe uma fila canônica `Revisar e enviar`; cada cartão possui uma única porta `Revisar e enviar ao cliente`. Roteiro, legenda, referências, comentário, devolução à Gabi e publicação ficam no mesmo painel expandido. Não nasceu uma segunda fila nem foi transferida responsabilidade.

> **LEI V115:** ajuste não é exclusão nem autoriza tela vazia. A última versão publicada continua consultável, enquanto a versão em correção permanece interna. Gravação, vídeo e legenda carregam cliente, competência e `itemId` da mesma pauta; divergência aparece e bloqueia sucesso falso antes da escrita. Cada papel recebe uma próxima ação visível, não acesso amplo.

**Gate local V115:** foco 31/31; UI focal de referências 10/10 em Chromium desktop/mobile; V114 28/28; V113 23/23; Firestore Emulator V113 15/15 e `expression_limit_hits=0`; Chromium real V113 26/26, V114 19/19, captação V99 57/57, Gerência/Amanda V112 27/27 e calendários V89 16/16; captação V99 80/80, privacidade V110 49/49, Calendários V87 50/50 e crítica 610/610. `firestore.rules` e os três módulos financeiros permanecem byte a byte V114. Nenhum dado real ou Firebase de produção foi escrito.
