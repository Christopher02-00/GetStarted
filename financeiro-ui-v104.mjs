/*
 * Get Started — integração financeira V104.
 *
 ...
 */
import * as Core from './financeiro-core.mjs?v=104';
...
function competenciaDeDataCaixa(valor){
  ...
  /* V104 — a Régua voltou a ser uma lista curta de ação. Julho e agosto
     são o período histórico confirmado pelo Chris e não viram atraso. */
  w.renderCobranca=async function(){
    if(!canFinanceiro()){ document.getElementById('cobrancaBox')?.replaceChildren(); return false; }
    const box=document.getElementById('cobrancaBox'); if(!box)return false;
    const competencia=campoCompetencia('cobMes');
    ...
    try{
      ...
      const linha=(v,{previsao=false}={})=>{
        ...
        const cobradoHoje=texto(v.ultimaCobranca).slice(0,10)===hoje;
        const cobravel=c.estado!=='a_vencer'&&c.estado!=='indisponivel';
        const editor=`<div id="editarWhatsV104_${escAttr(v.id)}" ${telefone?'hidden':''} style="display:flex;gap:6px;align-items:center;margin-top:7px;"><input id="whatsRapidoV103_${escAttr(v.id)}" value="${escAttr(telefoneParaTela(telefone))}" placeholder="(41) 99999-9999" style="flex:1;min-width:150px;"><button class="btn secondary" style="width:auto;" onclick="salvarWhatsRapidoV103('${escJs(v.id)}','${escJs(v.canonicalId)}')">Salvar contato</button></div>`;
        const contato=`<div class="meta" style="margin-top:5px;">WhatsApp: <b>${telefone?esc(telefoneParaTela(telefone)):'não cadastrado'}</b>${telefone?` · <button class="linkBtn" onclick="alternarEdicaoWhatsV104('${escJs(v.id)}')">editar</button>`:''}</div>${editor}`;
        const acao=previsao
        ...
        return `<div class="item" data-regua-linha="${escAttr(v.id)}" style="margin-top:7px;${cobradoHoje?'opacity:.72;':''}"><div class="top"><div class="nome">${esc(nomeLinha(v,nomes))}</div><span class="selo ${cobradoHoje?'aprovada':classeEstadoCobranca(c.estado)}">${cobradoHoje?'✔ cobrado hoje':esc(rotuloEstadoCobranca(c.estado,c.dias))}</span></div><div class="meta">${brl(v.valorDevido)} · ${esc(nomeMes(v.competencia))}${v.materializada?'':' · ainda não criada'}</div>${contato}${acao}</div>`;
      };
      const comprovantes=reg.comprovantesEmAnalise?.itens||[];
      const acionaveis=[...(reg.anterioresVencidos.itens||[]),...(reg.competenciaSelecionada.itens||[])].filter(v=>{
        const c=v.cobranca||Core.classificarCobranca(v,hoje,5);
  ...
  function fecharVigenciaNaCompetencia(contrato,competenciaFinal){
    ...
    const vigenciasInformadas=Array.isArray(contrato?.vigencias)
      ? contrato.vigencias.filter(v=>v&&v.excluido!==true)
      : [];
    ...
    let fechou=false;
    const vigencias=vigenciasInformadas.map(v=>{
  ...
  function manterVigenciaFedaltoAtiva(contrato){
    ...
    if(!salvas.length){
      return Core.competenciaValida(inicio)
        ? [{inicio,fim:'',valor:numero(contrato.valorInicial??contrato.valorVigente??contrato.valorCheio),cicloId:'legado-inicial'}]
        : null;
    }
    const abertas=salvas.filter(v=>!texto(v.fim));
    if(abertas.length===1&&texto(abertas[0].inicio)<='2026-09')return salvas;
    if(abertas.length)return null;
    const encerradasEmSetembro=salvas.filter(v=>texto(v.fim)==='2026-09'&&texto(v.inicio)<='2026-09');
    if(encerradasEmSetembro.length!==1||salvas.some(v=>texto(v.inicio)>'2026-09'))return null;
    return salvas.map(v=>v===encerradasEmSetembro[0]?{...v,fim:''}:v);
  }
  ...
  w.aplicarCorrecaoFinanceiraSetembroV103=async function(){
    ...
    if(!w.__aplicacaoFinanceiraV104EmCurso&&!confirm('Aplicar Vitalle, saída da Monique, deduplicação do Joaquim e contato da Zeiss? O sistema preservará pagos, cancelados e históricos.'))return false;
  ...
  async function lerCorrecaoFedaltoV104(){
    ...
    if(fedalto&&!vigencias)bloqueios.push('O ciclo da Fedalto está ambíguo e não pode ser reaberto automaticamente');
    ...
    if(fedalto&&vigencias){
      ...
      if(projecao.estado==='indisponivel'||ativosSimulados!==19)bloqueios.push(`A carteira final simulada resultou em ${ativosSimulados??'estado indisponível'} ativo(s), não 19`);
    }
    return {fontes,grupo,fedalto,vigencias,pagamentos,pagamento,estado,saidas,config,configRegua,ativosSimulados,bloqueios};
  }
  ...
  w.preverCorrecaoFedaltoReguaV104=async function(){
    ...
    try{
      ...
      alvo.innerHTML=`${p.bloqueios.length?`<div class="desc" style="color:var(--red);"><b>Correção bloqueada:</b> ${p.bloqueios.map(esc).join(' · ')}</div>`:`<div class="desc" style="color:var(--green);"><b>Prévia confirmada. Setembro fecha com ${p.ativosSimulados} clientes ativos; nada foi gravado.</b></div>`}<div class="item"><b>Fedalto Eletro</b><div class="meta">Permanece cliente ativo. Setembro de 2026 será cortesia promocional; agosto e os demais meses não serão reclassificados.</div></div><div class="item"><b>Régua de cobrança</b><div class="meta">A operação começa em julho de 2026 e julho/agosto ficam confirmados como período encerrado, sem inventar datas de caixa.</div></div>${p.bloqueios.length||w.__previewUnificadaV104?'':`<button class="btn" style="width:auto;margin-top:10px;" onclick="aplicarCorrecaoFedaltoReguaV104()">Corrigir Fedalto e a Régua</button>`}`;
      return !p.bloqueios.length;
  ...
  w.preverCorrecaoFinanceiraV104=async function(){
    ...
    try{
      ...
      if(alvo)alvo.innerHTML=carteiraOk&&fedaltoOk?'<button class="btn" style="width:auto;" onclick="aplicarCorrecaoFinanceiraV104()">Aplicar todas as correções confirmadas</button><div class="meta" style="margin-top:7px;">Uma confirmação; cada etapa possui recibo próprio e pode ser retomada sem duplicar.</div>':'<div class="desc" style="color:var(--red);">A aplicação continua bloqueada enquanto alguma prévia não estiver segura.</div>';
