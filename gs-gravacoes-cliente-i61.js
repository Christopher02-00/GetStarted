...
(function(global){
  ...
  function store({fb,uid,canEdit,monthOf,validateCalendar}){
    ...
    return {read,async save({cliente,mes,kind,id,values,base,source,operationId}){
      ...
      const result={itens:null,grupos:null};let disposed=false;
      const timer=setTimeout(()=>{if(!disposed&&(!result.itens||!result.grupos))onError(Error('O servidor ainda não confirmou a leitura. Tente novamente.'));},12000);
      const stops=['itens','grupos'].map(k=>fb.onSnapshot(fb.collection(fb.db,'gravacoes_cliente',c,'meses',m,k),{includeMetadataChanges:true},s=>{
        if(disposed||s.metadata?.fromCache)return;result[k]=Object.fromEntries(s.docs.map(d=>[d.id,d.data()]));if(result.itens&&result.grupos){clearTimeout(timer);cb(result);}
  ...
  async function reader(el,{cliente,mes,itens,store:db,open=false}){
    ...
    const render=r=>{
      ...
      const rows=itens.filter(i=>!i.excluido).map(i=>({i,r:r.itens[i.itemId],s:situacao(r.itens[i.itemId],hashes[i.itemId])}));
  ...
  global.GetGravacoesI61={esc,assinatura,situacao,valores,validar,store,reader,stopReader,editor,mesValido,style};
})(window);
