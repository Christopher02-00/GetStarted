...
const projeto = "demo-get-financeiro-v103";
const javaHome = process.env.GET_V103_JAVA_HOME || process.env.GET_V101_JAVA_HOME || "/private/tmp/get-v101-jdk21/Contents/Home";
const java = join(javaHome, "bin", "java");
const firebaseLocal = join(aqui, "node_modules", ".bin", "firebase");
const firebaseV101 = join(aqui, "..", "firebase-emulator-v101", "node_modules", ".bin", "firebase");
const firebase = existsSync(firebaseLocal) ? firebaseLocal : firebaseV101;
const regrasCanonicas = join(aqui, "..", "..", "firestore.rules");
const regrasGeradas = join(aqui, "rules-v103.generated.rules");
const logFirestore = join(aqui, "firestore-debug.log");

if (!existsSync(java)) {
  console.error(`JDK V103 indisponível em ${java}. Defina GET_V103_JAVA_HOME para um JDK 21 local.`);
  process.exit(2);
}
...
const env = {
  ...
  JAVA_HOME: javaHome,
  GET_V103_DEPS_ROOT: existsSync(join(aqui, "node_modules"))
    ? aqui
    : join(aqui, "..", "firebase-emulator-v101"),
  PATH: `${join(javaHome, "bin")}:${dirname(process.execPath)}:${process.env.PATH || ""}`
};

const comandoTeste = `"${process.execPath}" "${join(aqui, "rules-v103-financeiro.test.mjs")}"`;
let resultado;
try {
...
console.log(`V103_FINANCEIRO_EMULATOR_BUDGET expression_limit_hits=${estourosDeOrcamento}`);
