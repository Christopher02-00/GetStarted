...
const obrigatorios = [
  ...
  'scripts/regression-v101-ui-controle-conclusao.mjs',
  'scripts/regression-v103-financeiro-competencias.mjs',
  ...
  'scripts/regression-v108-conflitos-fedalto.mjs',
...
const leisV77 = [
  'lead comercial é identificado por origem e ID, e sua projeção no funil é idempotente',
...
for (const arquivo of htmls) {
  ...
  const declaradas = new Set([...fonteHandlers.matchAll(/(?:function\s+|window\.|\bw\.)([A-Za-z_$][\w$]*)/g)].map(m => m[1]));
...
for (const colecao of ['pagamentos_extra', 'pagamentos_mensais', 'calendarios', 'clientes_config']) {
  ...
  const proximo = inicio >= 0 ? regras.indexOf('\n    match /', inicio + 1) : -1;
  const trecho = inicio >= 0 ? regras.slice(inicio, proximo >= 0 ? proximo : regras.length) : '';
...
if (build !== '2026-08-23-pagamento-fedalto-agosto-v108' ||
    !escritorio.includes('<meta name="gs-parent-patch" content="2026-08-22-estado-conciliacao-joaquin-v107">') ||
    !escritorio.includes('<meta name="gs-grandparent-patch" content="2026-08-22-saida-canonica-joaquin-v106">') ||
    !escritorio.includes('<meta name="gs-great-grandparent-patch" content="2026-08-22-conciliacao-manual-regua-v105">') ||
    !escritorio.includes('<meta name="gs-fourth-grandparent-patch" content="2026-08-22-correcao-financeiro-real-v104">') ||
    !escritorio.includes('<meta name="gs-fifth-grandparent-patch" content="2026-08-21-financeiro-por-competencia-v103">') ||
    !escritorio.includes('<meta name="gs-sixth-grandparent-patch" content="2026-08-21-itemids-calendarios-legados-v102">') ||
    !escritorio.includes('<meta name="gs-seventh-grandparent-patch" content="2026-08-21-controle-conclusao-calendarios-v101">') ||
    !escritorio.includes('<meta name="gs-base-patch" content="2026-08-19-rodrigo-so-edicao-v91-1">')) {
  falhar(`cadeia de build V108 inesperada: ${build || 'ausente'}`);
} else provar('V108 preserva V107/V106/V105/V104/V103/V102/V101 e mantém pagamento explícito, estado visual, conciliação explícita, reconciliação manual, correção real, finanças, migração e conferência separadas');
...
const carregarAvaliacaoPortal = avaliacaoPortal.slice(0, avaliacaoPortal.indexOf('window.salvarAvaliacaoCliente'));
...
} else provar('primeira avaliação consulta somente o slug da sessão e não depende de ler documento inexistente');
