...
/*
 * Regressão dirigida V103 — correção financeira de setembro.
 *
...
import { instalarFinanceiroV103 } from '../financeiro-ui-v103.mjs';

const raiz = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const fonteUi = fs.readFileSync(path.join(raiz, 'financeiro-ui-v103.mjs'), 'utf8');
...
function verificar(condicao, mensagem) {
  total += 1;
  assert.ok(condicao, `V103 CORRECAO FINANCEIRA: ${mensagem}`);
  console.log('PASS ', mensagem);
}

function igual(atual, esperado, mensagem) {
  total += 1;
  assert.deepEqual(atual, esperado, `V103 CORRECAO FINANCEIRA: ${mensagem}`);
  console.log('PASS ', mensagem);
}
...
function instalarRuntime(db, papel = 'Chris') {
  ...
  const api = {
    prever: globalThis.preverCorrecaoFinanceiraSetembroV103,
    aplicar: globalThis.aplicarCorrecaoFinanceiraSetembroV103,
  };
...
function fixtureInicial() {
  ...
  const demais = Array.from({ length: 18 }, (_, indice) => [
...
async function testarAplicacaoCompleta() {
  ...
  igual(globalThis.__correcaoSetembroV103?.ativosSimulados, 19, 'prévia real calcula 19 ativos em setembro');
  ...
  igual(documentosDaColecao(db, 'clientes_ciclo_financeiro').length, 4, 'quatro eventos append-only identificam os quatro ajustes');
  igual(db.commits.filter(v => v.tipo === 'transaction').length, 1, 'todos os ajustes pertencem a um único commit atômico');
...
async function testarDuasAbas() {
  ...
  igual(db.commits.filter(v => v.tipo === 'transaction').length, 1, 'duas abas produzem somente um commit com alterações');
  igual(documentosDaColecao(db, 'clientes_ciclo_financeiro').length, 4, 'duas abas não duplicam eventos');
  igual((obter(db, 'contratos_cliente/vitalle-odonto').historicoAlteracoesValor || []).filter(v => v.operationId === 'fin_v103_vitalle_2026_09').length, 1, 'duas abas não duplicam o histórico da Vitalle');
...
console.log(`REGRESSAO V103 CORRECAO FINANCEIRA REAL: OK (${total} verificacoes)`);
