...
/*
 * Regressão de interface V104 — Financeiro por competência.
 *
...
const evidencias = path.resolve(
  ...
  process.env.GET_V104_UI_EVIDENCE_DIR ||
  fs.mkdtempSync('/private/tmp/get-v104-ui-financeiro-'),
);
...
function trecho(inicio, fim) {
  ...
  if (a < 0 || b < 0) throw new Error(`V104 UI: trecho real ausente: ${inicio}`);
...
const fixtureInicial = {
  ...
  config_financeiro: {
    regua_cobranca: {
      ...
      criterioHistorico: 'fixture_sintetica_confirmada',
    },
  },
  clientes_config: {},
  contatos_clientes_financeiro: {
    zeiss: { slug: 'zeiss', nome: 'Zeiss · fixture sintética', whatsapp: '5541999999999' },
    'cliente-sintetico-01': { slug: 'cliente-sintetico-01', nome: 'Cliente Sintético 01', whatsapp: '5541999990001' },
  },
};
...
body{padding:64px 14px 24px}.fixtureMarcaV104{position:fixed;z-index:2147483647;top:7px;left:7px;right:7px;max-width:1160px;margin:auto;border:2px solid var(--yellow,#ffbc13);border-radius:10px;background:#141416;color:var(--yellow,#ffbc13);padding:9px 12px;font:900 11px/1.35 system-ui;letter-spacing:.065em;text-align:center}
.fixtureShellV104{width:min(1160px,100%);margin:0 auto}.view{display:none!important;position:static!important;width:100%!important;height:auto!important;min-height:0!important;overflow:visible!important}.view.fixtureAtiva{display:block!important}
@media(max-width:600px){body{padding:68px 7px 14px}.fixtureMarcaV104{font-size:9px}.fixtureShellV104{width:100%}.row2,.row3,.painelResumo{grid-template-columns:1fr!important}}
</style></head><body>
<div class="fixtureMarcaV104">FIXTURE SINTÉTICA V104 · FINANCEIRO POR COMPETÊNCIA · SEM FIREBASE REAL · SEM DADOS REAIS</div>
<main class="fixtureShellV104">${viewsFinanceiras}</main><div id="toast"></div>
<script>
window.__fixtureOriginal=${fixtureJson};
window.__usuarioV104='Chris';
...
window.__resetFixture=()=>{window.__store=__reviver(window.__fixtureOriginal);window.__fixtureWrites=[];window.__fixtureReads=[];window.__fixtureFalha='';window.__fixtureToasts=[];window.__fixtureGetDocFailure=null;window.__cobrancasPreparadas={};window.__usuarioV104='Chris'};
...
window.abrirCobranca=async id=>{const linha=window.__cobrancasV104?.[id];if(!linha?.materializada)return false;window.__cobrancasPreparadas[id]={cliente:linha.canonicalId,clienteNome:linha.clienteNome||'',fase:'fixture_vencida',preparadaEm:Date.now()};const botao=document.getElementById('confirmarCobranca_'+id);if(botao)botao.hidden=false;return true};
...
<script type="module">
import { instalarFinanceiroV104 } from '/financeiro-ui-v104.mjs?v=104';
window.__financeiroInstalado=instalarFinanceiroV104({
  ...
  usuarioAtual:()=>window.__usuarioV104,auth:window.auth,registrarLogAutomacao:window.registrarLogAutomacao,
});
...
exigir(fonteHtml.includes('./financeiro-core.mjs?v=105') && fonteUi.includes("./financeiro-core.mjs?v=105"), 'HTML e UI preservam a prova V104 sobre o mesmo núcleo financeiro V105');
...
async function selecionar(page, competencia, origem) {
  await page.evaluate(async ({ competencia, origem }) => {
    await window.sincronizarCompetenciaFinanceiraV104(competencia, origem);
  }, { competencia, origem });
}
...
async function testarSuperficies(page, vp) {
  ...
  const estado = await page.evaluate(async () => {
    const fontes = await window.__financeiroV104.carregarSnapshot();
    const agosto = window.__financeiroV104.projetar(fontes, '2026-08', { regua: false });
    const setembro = window.__financeiroV104.projetar(fontes, '2026-09', { regua: false });
    const setembroRegua = window.__financeiroV104.projetar(fontes, '2026-09', { regua: true });
    return {
      ...
      regua: {
        anterioresQuantidade: setembroRegua.regua.anterioresVencidos.quantidade,
        inicioOperacao: setembroRegua.regua.competenciaInicialOperacao,
        quitadasAte: setembroRegua.regua.competenciasQuitadasAte,
  ...
  exigir(reguaTexto.includes('cobrança(s) para cuidar agora') && reguaTexto.includes('Próxima competência'), `${vp}: Régua prioriza uma lista curta de ação e recolhe a previsão`);
  exigir(reguaTexto.includes('Julho e agosto de 2026 estão encerrados') && estado.regua.inicioOperacao === '2026-07' && estado.regua.quitadasAte === '2026-08', `${vp}: Régua comunica o marco real de início e o corte histórico confirmado`);
  exigir(estado.regua.anterioresQuantidade === 0 && await page.locator('#cobrancaBox [data-regua-linha="cliente-sintetico-01_2026-08"]').count() === 0, `${vp}: mensalidade aberta de agosto não reaparece como falso atraso`);
  exigir(estado.regua.anterioresSomenteReais && estado.regua.historicoVirtualAusente, `${vp}: Régua não fabrica obrigação histórica virtual`);
  exigir(estado.regua.comprovanteSeparado && estado.regua.comprovanteForaDoMes && reguaTexto.toLowerCase().includes('comprovante(s) aguardando conferência'), `${vp}: comprovante pendente fica separado da fila de cobrança`);
  exigir(reguaTexto.includes('🧾 1 comprovante(s) aguardando conferência'), `${vp}: Régua contabiliza um comprovante sintético em análise sem criar card redundante`);
  ...
  exigir(reguaTexto.includes('Somente previsão; abrir não cria cobrança.'), `${vp}: próxima competência não oferece ação de cobrança`);
  exigir(reguaTexto.includes('WhatsApp: (41) 99999-0001'), `${vp}: contato existente aparece diretamente na linha da Régua`);
  exigir(reguaTexto.includes('WhatsApp: não cadastrado') && reguaTexto.includes('Salvar contato'), `${vp}: contato ausente pode ser cadastrado sem sair da Régua`);
  ...
  await page.evaluate(() => window.mudarMesContratosV104(1));
...
async function testarErrosConflitosAcoes(page, vp) {
  await page.evaluate(() => { window.__financeiroV104.invalidar(); window.__fixtureFalha = 'permission-denied'; });
  ...
  await page.evaluate(() => { window.__financeiroV104.invalidar(); window.__fixtureFalha = 'deadline-exceeded'; });
  ...
  await page.evaluate(() => { window.__financeiroV104.invalidar(); window.__fixtureFalha = ''; });
  ...
  const linhasInvalidas = await page.evaluate(() => ({
    status: window.__mensalidadesV104?.['cliente-sintetico-07_2026-09'],
    valor: window.__mensalidadesV104?.['cliente-sintetico-08_2026-09'],
  }));
  ...
  const vitalleVirtual = await page.evaluate(() => window.__cobrancasV104['vitalle-odonto_2026-09']);
  ...
  const materializouCobranca = await page.evaluate(() => window.materializarCobrancaV104('vitalle-odonto_2026-09'));
  ...
  exigir(await page.evaluate(() => window.confirmarEnvioCobrancaV104('vitalle-odonto_2026-09')) === true, `${vp}: confirmar envio grava recibo da cobrança`);
  ...
  const retryCobranca = await page.evaluate(async () => {
    ...
    const ok = await window.confirmarEnvioCobrancaV104('vitalle-odonto_2026-09');
  ...
  await page.evaluate(() => { window.__fixtureWrites = []; window.__financeiroV104.invalidar(); return window.renderMensalidades(); });
  const mensalidadeVirtual = await page.evaluate(() => window.__mensalidadesV104['cliente-sintetico-07_2026-09']);
  ...
  const materializouPagamento = await page.evaluate(() => window.marcarMensalidadeV104('cliente-sintetico-07_2026-09', 'pago'));
  ...
  exigir(await page.evaluate(() => window.salvarLancamentoFinanceiroV104()) === true, `${vp}: salário explícito grava lançamento e evento atômicos`);
...
async function testarContratoWriterECaixa(page, vp) {
  ...
  const baixaSemData = await page.evaluate(async () => {
    ...
    const antes = window.__fixtureWrites.length;
    const ok = await window.alterarLancamentoFinanceiroV104('custo-agosto-previsto', 'pago');
...
async function testarPapeis(page, vp) {
  await page.evaluate(() => { window.__usuarioV104 = 'Amanda'; window.__fixtureReads = []; window.__financeiroV104.invalidar(); });
  ...
  for (const papel of ['Cecília','Gabrielle','Luís','Nathan']) {
    const r = await page.evaluate(async papel => {
      window.__usuarioV104 = papel; window.__fixtureReads = []; window.__financeiroV104.invalidar();
  ...
  await page.evaluate(() => { window.__usuarioV104 = 'Chris'; window.auth.currentUser = { uid: 'uid-chris-sintetico' }; window.__financeiroV104.invalidar(); });
}

async function evidencia(page, viewId, nome, rotulo) {
  await page.evaluate(id => window.__ativarView(id), viewId);
  exigir(await page.locator('.fixtureMarcaV104').isVisible(), `${rotulo}: selo sintético está visível`);
...
try {
  ...
  await evidencia(desktop.page, 'view-financeiro', 'V104_DESKTOP_FINANCEIRO_FIXTURE_SINTETICA.png', 'desktop/Financeiro');
  await pageRender(desktop.page, 'renderCobranca');
  await evidencia(desktop.page, 'view-cobranca', 'V104_DESKTOP_REGUA_FIXTURE_SINTETICA.png', 'desktop/Régua');
...
try {
  ...
  await evidencia(mobile.page, 'view-mensalidades', 'V104_MOBILE_MENSALIDADES_FIXTURE_SINTETICA.png', 'mobile/Mensalidades');
  await mobile.page.evaluate(() => window.renderContratos());
  await evidencia(mobile.page, 'view-contratos', 'V104_MOBILE_CONTRATOS_FIXTURE_SINTETICA.png', 'mobile/Contratos');
...
exigir(sha256Arquivo(caminhoUi) === hashesFonteInicial.ui, 'financeiro-ui-v104.mjs permaneceu no mesmo hash durante toda a prova');

console.log(`V104 UI FINANCEIRO: ${total - falhas.length}/${total} verificações aprovadas.`);
...
if (falhas.length) {
  console.error(`V104 UI FINANCEIRO: ${falhas.length} falha(s) real(is).`);
  process.exitCode = 1;
}
