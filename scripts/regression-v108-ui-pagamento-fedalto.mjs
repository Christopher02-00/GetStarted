...
/*
 * Regressão UI V108 — pagamento de agosto da Fedalto.
 *
...
function bootstrapFixtureV108() {
  const parametros = new URLSearchParams(location.search);
  window.usuarioAtual = parametros.get('role') || 'Chris';
  window.db = { path: '' };
  window.auth = { currentUser: { uid: 'uid-sintetico-v108' } };
  ...
  const storageKey = `fixture-v108-fedalto:${runId}`;
    ...
    return {
      ...
      pagamentos_mensais: {
        'fedalto-eletro-comercial_2026-07': {
          ...
          marcadorPreservado: { origem: 'fixture-v108', ordem: 7 },
        },
        ...
        'fedalto-eletro-comercial_2026-09': {
          ...
          marcadorPreservado: { origem: 'fixture-v108', ordem: 9 },
        },
      },
      clientes_encerrados: {},
  ...
  window.__assinaturaFixtureV108 = assinatura;
...
const htmlHarness = [
  ...
  'body{padding:18px}.fixtureV108{width:min(980px,100%);margin:auto}.fixtureV108 *{box-sizing:border-box}',
  '@media(max-width:600px){body{padding:8px}.fixtureV108{width:100%}.row2,.row3,.painelResumo{grid-template-columns:1fr!important}}',
  '</style></head><body><main class="fixtureV108">',
  ...
  '</main><script>(', bootstrapFixtureV108.toString(), ')();</script>',
  '<script type="module">',
  ...
  'window.__runtimeV108=instalarFinanceiroV104(deps);',
  'window.__stubRendersV108=()=>{window.renderFinanceiro=async()=>{window.__fixtureRenderCalls.push("financeiro");return true;};window.renderMensalidades=async()=>{window.__fixtureRenderCalls.push("mensalidades");return true;};window.renderCobranca=async()=>{window.__fixtureRenderCalls.push("cobranca");return true;};window.renderContratos=async()=>{window.__fixtureRenderCalls.push("contratos");return true;};};',
...
async function dimensoesPainel(page) {
  return page.evaluate(() => {
    const painel = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    return {
...
function exigirSemOverflow(largura, rotulo) {
  exigir(
    ...
    `${rotulo}: painel V108 permanece sem overflow horizontal`,
  );
}

async function testarFluxoPrincipal(page, rotulo) {
  const inicial = await page.evaluate(() => ({
    presente: !!document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status'),
    estado: document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status')?.dataset?.estado || '',
  ...
  const previaOk = await page.evaluate(() => window.preverCorrecaoFedaltoAgostoV108());
  await page.waitForFunction(() => document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status')?.dataset?.estado === 'pronta');
  const previa = await page.evaluate(() => {
    const alvo = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
  ...
  exigir(previa.botoes === 1 && previa.botao.includes('Confirmar pagamento de 15/08/2026'), `${rotulo}: escrita exige um único botão explícito com a data humana confirmada`);
  ...
  const antes = await page.evaluate(() => ({
    julho: window.__assinaturaFixtureV108(window.__store.pagamentos_mensais['fedalto-eletro-comercial_2026-07']),
    setembro: window.__assinaturaFixtureV108(window.__store.pagamentos_mensais['fedalto-eletro-comercial_2026-09']),
    config: window.__assinaturaFixtureV108(window.__store.clientes_config['fedalto-eletro-comercial']),
  }));
  await page.evaluate(() => window.__stubRendersV108());
  await page.click('#financeiroCorrecaoFedaltoAgostoV108Status button');
  await page.waitForFunction(() => document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status')?.dataset?.estado === 'resolvida');
  ...
  const depois = await page.evaluate(antesFixture => {
    ...
    const evento = s.clientes_ciclo_financeiro.fin_v108_fedalto_agosto_20260815;
    const commits = window.__fixtureWrites.filter(item => item.tipo === 'transaction');
    const alvo = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    return {
      julhoPreservado: window.__assinaturaFixtureV108(s.pagamentos_mensais['fedalto-eletro-comercial_2026-07']) === antesFixture.julho,
      setembroPreservado: window.__assinaturaFixtureV108(s.pagamentos_mensais['fedalto-eletro-comercial_2026-09']) === antesFixture.setembro,
      configPreservada: window.__assinaturaFixtureV108(s.clientes_config['fedalto-eletro-comercial']) === antesFixture.config,
      contrato,
      agosto,
  ...
  exigir(depois.commits === 1 && depois.writes === 1 && JSON.stringify(depois.caminhos) === JSON.stringify([
    'clientes_ciclo_financeiro/fin_v108_fedalto_agosto_20260815',
    'contratos_cliente/fedalto-eletro-comercial',
    'pagamentos_mensais/fedalto-eletro-comercial_2026-08',
  ]), `${rotulo}: clique faz uma transação atômica somente em contrato, agosto e ledger`);
  ...
  exigir(depois.contrato?.financeiroRevision === 5 && depois.contrato?.financeiroOperationId === 'fin_v108_fedalto_agosto_20260815', `${rotulo}: contrato recebe revisão monotônica e operationId determinístico`);
  ...
  exigir(depois.evento?.operationId === 'fin_v108_fedalto_agosto_20260815' && depois.evento?.clienteId === 'fedalto-eletro-comercial' && depois.evento?.sourceType === 'contrato' && depois.evento?.dataEfetivaIso.startsWith('2026-08-15T15:00:00.000Z'), `${rotulo}: ledger append-only registra identidade, fonte e data efetiva esperadas`);
  exigir(depois.julhoPreservado && depois.setembroPreservado && depois.configPreservada, `${rotulo}: julho, setembro e ficha permanecem byte a byte`);
  exigir(depois.confirma.length === 1 && depois.confirma[0].includes('preservando julho') && depois.confirma[0].includes('setembro como cortesia'), `${rotulo}: confirmação humana repete o impacto antes do commit`);
  ...
  const retry = await page.evaluate(async () => {
    ...
    const ok = await window.aplicarCorrecaoFedaltoAgostoV108();
    const estado = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    return {
      ok,
      antesWrites,
  ...
  const reload = await page.evaluate(() => {
    ...
    return {
      ...
      evento: window.__store.clientes_ciclo_financeiro.fin_v108_fedalto_agosto_20260815 || null,
    };
  });
  ...
  exigir(reload.contratoInicio === '2026-07' && reload.agosto?.pagoEm === '2026-08-15' && reload.evento?.operationId === 'fin_v108_fedalto_agosto_20260815', `${rotulo}: contrato, agosto e ledger persistem depois da recarga`);
  ...
  const futuro = await page.evaluate(async () => {
    ...
    window.__runtimeV108.invalidar();
    const antesWrites = window.__fixtureWrites.length;
  ...
  exigir(futuro.ok && futuro.writes === 0 && futuro.estado === 'resolvida' && futuro.selo === 'Concluído' && futuro.botoes === 0, `${rotulo}: uma saída financeira futura legítima não reabre nem repete a conciliação histórica V108`);

  const reciboDivergente = await page.evaluate(async () => {
    const evento = window.__store.clientes_ciclo_financeiro.fin_v108_fedalto_agosto_20260815;
  ...
  exigir(reciboDivergente.ok && reciboDivergente.writes === 0 && reciboDivergente.estado === 'bloqueada' && reciboDivergente.texto.includes('recibo V108 existe'), `${rotulo}: recibo divergente nunca é apresentado silenciosamente como concluído`);
}

async function testarPapelIndevido(page, papel) {
  const resultado = await page.evaluate(async () => {
    const presenteAntes = !!document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    const antesLeituras = window.__fixtureReads.length;
    const antesWrites = window.__fixtureWrites.length;
    const previa = await window.preverCorrecaoFedaltoAgostoV108();
    const aplicacao = await window.aplicarCorrecaoFedaltoAgostoV108();
    const alvo = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    return {
      previa,
      aplicacao,
...
async function testarPrecondicaoBloqueada(page, mutacao, trechoEsperado, rotulo) {
  const resultado = await page.evaluate(async ({ mutacao, trechoEsperado }) => {
    ...
    const antesWrites = window.__fixtureWrites.length;
    const ok = await window.preverCorrecaoFedaltoAgostoV108();
    const alvo = document.getElementById('financeiroCorrecaoFedaltoAgostoV108Status');
    return {
      ok,
      writes: window.__fixtureWrites.length - antesWrites,
      ...
      botoesConfirmacao: [...(alvo?.querySelectorAll('button') || [])]
        .filter(botao => botao.textContent.includes('Confirmar pagamento')).length,
    };
...
exigir(fonteHtml.includes('2026-08-23-pagamento-fedalto-agosto-v108'), 'HTML real identifica o build V108 da Fedalto');
exigir(fonteHtml.includes('financeiro-core.mjs?v=108') && fonteHtml.includes('financeiro-ui-v104.mjs?v=108'), 'HTML real instala core e UI com cache-buster V108');
exigir(!fonteHtml.includes('financeiroCorrecaoFedaltoAgostoV108Status'), 'HTML público não contém o alvo nem fatos financeiros estáticos da V108');
exigir(fonteUi.includes('preverCorrecaoFedaltoAgostoV108') && fonteUi.includes('aplicarCorrecaoFedaltoAgostoV108'), 'UI real expõe prévia e confirmação separadas');
exigir(fonteUi.includes('garantirPainelCorrecaoFedaltoAgostoV108') && fonteUi.includes("if(!canFinanceiro()){existente?.remove();return null;}"), 'UI cria o painel V108 somente depois de confirmar o papel Financeiro');
exigir(fonteUi.includes("operationId:'fin_v108_fedalto_agosto_20260815'") && fonteUi.includes("augustPaidAt:'2026-08-15'"), 'UI real fixa identidade idempotente e data civil confirmada');
...
try {
  ...
  for (const caso of [
    ...
    { mutacao: 'setembro-cortesia', trecho: 'Setembro deixou de ser a cortesia promocional', rotulo: 'pré-condição de setembro' },
  ]) {
...
console.log(`V108 UI PAGAMENTO FEDALTO: ${total - falhas.length}/${total} verificações aprovadas.`);
...
if (falhas.length) {
  console.error(`V108 UI PAGAMENTO FEDALTO: ${falhas.length} falha(s) real(is).`);
  process.exitCode = 1;
}
