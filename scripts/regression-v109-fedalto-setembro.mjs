...
/* Regressão V109 — pagamento de agosto e cortesia canônica da Fedalto. */
import assert from 'node:assert/strict';
import * as Core from '../financeiro-core.mjs';

let total=0;
function ok(condicao,mensagem){total+=1;assert.ok(condicao,`V109: ${mensagem}`);console.log('PASS ',mensagem);}
function igual(atual,esperado,mensagem){total+=1;assert.deepEqual(atual,esperado,`V109: ${mensagem}`);console.log('PASS ',mensagem);}
...
const contratoAntes={
  ...
  cortesiaMeses:['2026-09'],vigencias:[{inicio:'2026-09',fim:'',valor:1700,cicloId:'fixture-v109'}],
};
const pagamentosAntes=[
  ...
  {id:'fedalto-eletro-comercial_2026-09',canonicalId:'fedalto-eletro-comercial',cliente:'fedalto-eletro-comercial',competencia:'2026-09',status:'isento',pagoEm:'',cortesiaDoMes:false,motivoIsencao:'cortesia manual',valorDevido:1700,diaVencimento:10},
];
...
const contratoDepois={
  ...
  financeiroOperationId:'fin_v109_fedalto_agosto_setembro_20260815',
...
const pagamentosDepois=pagamentosAntes.map(v=>v.id==='fedalto-eletro-comercial_2026-08'?{
  ...
  motivoIsencao:undefined,financeiroOperationId:'fin_v109_fedalto_agosto_setembro_20260815',
}:v.id==='fedalto-eletro-comercial_2026-09'?{
  ...v,cortesiaDoMes:true,motivoIsencao:'Cortesia promocional da agência em setembro de 2026',
  financeiroOperationId:'fin_v109_fedalto_agosto_setembro_20260815',
}:v);
...
igual(linhaSetembro.pagoEm,'','setembro continua sem data de caixa');
igual(linhaSetembro.motivoIsencao,'Cortesia promocional da agência em setembro de 2026','setembro usa o motivo promocional canônico');
...
console.log(`OK V109 conflitos/Fedalto: ${total}/${total} verificações`);
