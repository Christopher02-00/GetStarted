...
exigir(escritorio.includes('<meta name="gs-build" content="2026-08-17-central-vendas-v77">'),'build V77 identificado');
...
exigir(mover.indexOf('Mensalista ainda não foi ativado')<mover.indexOf("setDoc(doc(db,'clientes_extras',slug)"),'mensalista sem ficha ativa é bloqueado antes de criar cliente avulso');
...
const central=trecho(escritorio,'/* ===== CENTRAL DE VENDAS — V77','  /* ===== FUNIL DE NEGOCIOS');
...
exigir(central.includes("await diagnosticarIdentidadeCliente(nome)")&&central.includes("clienteSlug:diag.existe?diag.slug:''"),'proposta cruza identidade existente sem cadastrar cliente');
...
exigir(escritorio.includes("onclick=\"editarPropostaNegocio('${n.id}')\""),'cartão comercial permite retomar a proposta diretamente');
