...
exigir(escritorio.includes("const arquivoUnico=new Map()")&&escritorio.includes("const chave=slugDo(v)||('registro-'+v.id)")&&escritorio.includes('if(!anterior || (v.origemCentralEntrada&&!anterior.origemCentralEntrada))'),'arquivo consolida por identidade e prefere a saída oficial');
...
exigir(!trecho('window.salvarClienteAtivoCentral','  window.ativarMensalistaRecebido').includes('numeroWhatsAppBrasil(dados.telefone)'),'edição compartilhada não sobrescreve a agenda privada com telefone legado');
...
exigir(listeners.includes("pessoaDoOuvinte==='Chris'")&&listeners.includes("['clientes_encerrados','clientes_config','contatos_clientes_financeiro']"),'somente Chris acompanha saídas e contatos para atualizar a planilha aberta');
...
const saidaCliente=trecho('window.salvarSaidaClienteCentral=async function','window.cancelarProgramacaoSaidaCentral=async function');
exigir(saidaCliente.includes("tx.set(refs.config,{...agenda,saidaAtivaId:encerRef.id,clienteInativo:imediata")&&saidaCliente.includes("tx.set(encerRef,{nome:atual.nome,slug,dataAviso,dataSaida"),'saída efetiva desativa a carteira e preserva a fotografia no mesmo fluxo');
