...
async function executar(viewport){
  ...
  try{
    ...
    await page.addScriptTag({content:`
      ...
      window.runTransaction=async(_db,fn)=>{ const ops=[]; const tx={get:async ref=>__snap(__banco.get(ref.path)),set:(ref,dados,opt)=>ops.push({ref,dados,merge:opt?.merge===true})}; await fn(tx); for(const op of ops){const atual=op.merge?(__banco.get(op.ref.path)||{}):{};__banco.set(op.ref.path,{...atual,...op.dados});} };
    ...
    await page.locator('#ctDia_iphone-campo-largo').fill('10');
    await page.locator('#ctPrimeiraCompetencia_iphone-campo-largo').fill('2026-08');
    await page.locator('#ctCortesiaMeses_iphone-campo-largo').fill('2026-08\n2026-09');
    ...
    await botao.click();
    exigir(await page.evaluate(()=>!__banco.has('contratos_cliente/iphone-campo-largo')&&!__banco.has('pagamentos_mensais/iphone-campo-largo_2026-08')),
    ...
    await botao.click();
    await page.waitForFunction(()=>window.__banco.has('contratos_cliente/iphone-campo-largo')&&window.__banco.has('pagamentos_mensais/iphone-campo-largo_2026-08'));
    const observado=await page.evaluate(()=>({contrato:__banco.get('contratos_cliente/iphone-campo-largo'),mensalidade:__banco.get('pagamentos_mensais/iphone-campo-largo_2026-08'),acesso:__banco.get('clientes_acesso/iphone-campo-largo'),toasts:__toasts}));
    exigir(observado.contrato.valorVigente===800&&observado.mensalidade.valorDevido===800,'clique real dispara a transação e propaga R$ 800 ao contrato e mensalidade');
